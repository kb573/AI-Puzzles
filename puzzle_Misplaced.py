    ...
    while 1:
        ...
        for x in transformations:
            if blank_tile not in x['position']:
                ...
                state_copy = deepcopy(initial)                   
                state_copy[blank_tile], state_copy[blank_tile + x['head']] = state_copy[blank_tile + x['head']], state_copy[blank_tile]             
                # check if a node has been previously explored.
                if ~(np.all(list(paths['initial']) == state_copy, 1)).any():    
                    end_time = time.time()
                    if (( end_time - start_time ) > 2):
                        print("There was an error solving this puzzle. \n")
                        exit 
                    # get the misplaced tiles estimate.
                    hn = misplaced_tiles(positions(state_copy), costg)    
                     # a new state is added to the paths list.                    
                    new_state = np.array([(state_copy, position, gn, hn)], dtstate)         
                    ...
                    if np.array_equal(state_copy, goal):                              

...
def manhattan_distance(initial, goal):
    """
    Calculate Manhattan distance cost from the initial state to the goal state. 

    Inputs: initial (the initial state), goal (the goal state)
    Return: sum(cost[1:]) (the sum of distance between initial tiles and goal tiles)
    """
    a = abs(initial // 3 - goal // 3)
    b = abs(initial % 3 - goal % 3)
    cost = a + b
    return sum(cost[1:])
...
def search(initial, goal):
    ...
    gn = 0
    hn = manhattan_distance(positions(initial), costg)
    parent = -1
            ...
            if blank_tile not in x['position']:
                ...
                if ~(np.all(list(paths['initial']) == openstates, 1)).any():    
                    ...
                    if (( end_time - start_time ) > 2):
                        ...
                        exit 
                    # get the manhatten estimate.
                    hn = manhattan_distance(positions(openstates), costg)    
...
paths, visited = search([7,2,4,5,0,6,8,3,1], [0,1,2,3,4,5,6,7,8]) 
